...
final class ConverterController: UIViewController {
    ...
    private func updateUI() {
        let values = viewModel.currentValues

        gbpCard.updateAmount(values.input)
        usdCard.updateAmount(values.output)
        
        let state = viewModel.currentDisplayState
        gbpCard.updateTitle(state.topTitle, flagName: state.topFlag)
        gbpCard.updateAmount(state.topAmount)
                
        usdCard.updateTitle(state.bottomTitle, flagName: state.bottomFlag)
        usdCard.updateAmount(state.bottomAmount)
    }
